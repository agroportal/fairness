...
public class F3 extends AbstractPrincipleCriterion {
	...
	protected void doEvaluation(Ontology ontology) throws JSONException, IOException, MalformedURLException, SocketTimeoutException {
		...
		this.addResult(1, this.questions.get(1).getMaxPoint().getScore(), "(TODO:change explanation) Ontology metadata are not included in an external file");
		//Q3: Does that external file explicitly link to the ontology and vice-versa?
		// TODO change explanation
		this.addResult(2, this.questions.get(2).getMaxPoint().getScore() / 2 , "(TODO:change explanation)  Agroportal make  explicit link from metadata to the ontology");
	}
